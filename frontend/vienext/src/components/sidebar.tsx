...
//   const { toggleTheme, setTheme: setStoreTheme } = useAppStore()
...
//     toggleTheme() // Cập nhật theme trong useAppStore
//     setTheme(newTheme) // Đồng bộ với next-themes
//     setStoreTheme(newTheme) // Đảm bảo store cũng được cập nhật
//   }
...
import { useAppStore } from "@/contexts/store";
import { useTheme } from "next-themes";
import { apiClient } from "@/lib/api"; // Giả định bạn đã có apiClient
...
export function Sidebar({ userPlan, userData }: SidebarProps) {
  ...
  const [showUserProfile, setShowUserProfile] = useState(false);
  const [showDetailedProfile, setShowDetailedProfile] = useState(false);
  const [showLogoutConfirm, setShowLogoutConfirm] = useState(false); // Trạng thái dialog xác nhận
  const { toggleTheme, setTheme: setStoreTheme } = useAppStore();
  const { language, setLanguage, t } = useAppStore();
  const { theme, setTheme } = useTheme();
  ...
  const handleLogout = async () => {
    try {
      ...
      router.push("/auth"); // Redirect về trang đăng nhập sau khi logout
    } catch (error) {
  ...
  return (
    <>
          ...
          >
            ...
            >
              <h3 className="text-lg font-semibold text-slate-900 dark:text-white">
                {t.confirmLogout || "Xác nhận đăng xuất"}
              </h3>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
                {t.logoutConfirmation || "Bạn có chắc chắn muốn đăng xuất không?"}
              </p>
              <div className="mt-4 flex justify-end space-x-2">
                <button
                  onClick={() => setShowLogoutConfirm(false)}
                  className="rounded bg-slate-200 px-4 py-2 text-sm font-medium text-slate-900 hover:bg-slate-300 dark:bg-slate-700 dark:text-white dark:hover:bg-slate-600"
                >
                  {t.cancel || "Hủy"}
                </button>
                <button
      ...
      {(showUserProfile || showDetailedProfile || showLogoutConfirm) && (
        <div className="fixed inset-0 z-20 bg-black/20 dark:bg-black/50" onClick={closeProfiles} />
      )}
    </>
  );
