...
export function PostDetailModal({ selectedPost, post, onClose, onAddComment }: PostDetailModalProps) {
  const [newComment, setNewComment] = useState("")

  if (!post || selectedPost === null) return null
  ...
  const handleSubmitComment = () => {
    ...
    onAddComment(selectedPost, newComment)
    setNewComment("")
  }

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative max-h-[90vh] max-w-4xl overflow-auto rounded-lg bg-white dark:bg-slate-900">
        <Button
          variant="ghost"
          size="icon"
          className="absolute right-2 top-2 z-10 h-8 w-8 rounded-full bg-slate-800/50 text-white hover:bg-slate-800/70"
          onClick={onClose}
        >
          <X className="h-4 w-4" />
        </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
          <div className="flex items-center justify-center bg-slate-100 p-4 dark:bg-slate-800">
            <img src={post.image} alt="Post" className="max-h-[80vh] w-auto object-contain" />
          </div>
              ...
              <div className="flex space-x-3">
                ...
                <div className="flex-1">
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && selectedPost !== null) {
                        handleSubmitComment()
                      }
                    }}
                  />
